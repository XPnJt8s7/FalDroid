...
def basic_info(G):
    ...
    f.write('Number of network nodes:')
    f.write(str(G.number_of_nodes()) + '\n')
    f.write('Number of network edges:')
    f.write(str(G.size()) + '\n')
    f.write('Network edge weighted sum:')
...
def convert_igraph(G):
    ...
    for e in G.edges_iter():
        ...
        # This statement is to determine whether there is an edge in g. 
        # If not, the label_propagation algorithm that follows will report an error.
        if(g.are_connected(s,d)==False):
            g.add_edges([(s,d)])
            #i=i+1
            #print i
    #print g
