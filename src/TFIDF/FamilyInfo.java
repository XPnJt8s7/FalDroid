...
public class FamilyInfo {
	...
	public void iniApkInfoList() {
		...
		if (!apktoolFile.exists()) {
			System.err.println("Directory " + apktoolFile.toString() + " doesn't exists, exiting");
			System.exit(-1);
		}
		// System.out.println("Checking in " + apktoolFile.toString());
	...
	public void writeMethodWeightFile(String writeString, Map<SensitiveMethod, Integer> allMap, int totalNum) {
		try {
			...
			for (int i = 0; i < methodWeightList.size(); i++) {
				...
				String lineString = entry.getKey().getString() + "#" + value + "#" + numInFal + "/" + sampleNumber + "#"
						+ numInAll + "/" + totalNum + "\n";
				bWriter.write(lineString);
			}
			bWriter.close();
			fWriter.close();
			System.out.println("[FamilyInfo] File saved as " + writeString);
		} catch (Exception e) {
	...
	public void writeMethodNumberFile(String writeString, Map<SensitiveMethod, Integer> allMap, int totalNum) {
		try {
			...
			for (int i = 0; i < methodNumberList.size(); i++) {
				...
				String lineString = method.getString() + "#" + number + "#" + number + "/" + this.sampleNumber + "#"
						+ number + "/" + totalNum + "\n";
				bWriter.write(lineString);
			}
			bWriter.close();
